...
df['nt_pickups'] = np.exp(-1.699553*df['airport'] + 0.5047479*df['tourist'] + 0.3486627*df['nt_linear_pickups_log'] + 1.082214*df['nt_dc_logsum'] - 0.0173114*df['median_age'] + 0.0170123*df['pct_bach_25p'] + 0.000000716*df['total_emp_den'] + 0.2991526 - 5.691818 + 0.2) # +0.1
df['am_pickups'] = np.exp(-0.7743786*df['airport'] + 0.4779832*df['tourist'] + 0.4050372*df['am_linear_pickups_log'] + 0.6230226*df['am_dc_logsum'] - 0.0262*df['median_age'] + 0.0143772*df['pct_bach_25p']+ 0.000000498*df['total_emp_den'] + 0.3775664 - 1.946639 + 0.25) # +0.18
df['md_pickups'] = np.exp(-0.6405087*df['airport'] + 0.7809523*df['tourist'] + 0.3927277*df['md_linear_pickups_log'] + 0.6977969*df['md_dc_logsum'] - 0.0208753*df['median_age'] + 0.0123951*df['pct_bach_25p']+ 0.00000161*df['total_emp_den'] + 0.3918755 - 2.551229 - 0.25)
df['pm_pickups'] = np.exp(-0.4855696*df['airport'] + 0.890186*df['tourist'] + 0.4090155*df['pm_linear_pickups_log'] + 0.8352944*df['pm_dc_logsum'] - 0.0216488*df['median_age'] + 0.0149257*df['pct_bach_25p']+ 0.00000217*df['total_emp_den'] + 0.363544 - 4.089885 - 0.45)
df['ev_pickups'] = np.exp(0.0188928*df['airport'] + 0.830838*df['tourist'] + 0.3445732*df['ev_linear_pickups_log'] + 0.9817033*df['ev_dc_logsum'] - 0.0215845*df['median_age'] + 0.0199614*df['pct_bach_25p']+ 0.00000224*df['total_emp_den'] + 0.4076045 - 5.265743 + 0.2) #+0.12
